...
export class UserLearningProfileStorage {
  ...
  async updateLanguageProfile(
    ...
    try {
      ...
      if (!profile) {
        logger.warn('Cannot update language profile - user profile not found', {
          userId,
          languageCode,
        });
        return;
  ...
  async updateStreakChallengeData(
    ...
    modeData.accuracy =
      (modeData.accuracy * modeData.count + sessionData.accuracy) / (modeData.count + 1);
    modeData.count++;
    ...
    tierData.accuracy =
      (tierData.accuracy * (tierData.wordsAttempted - sessionData.wordsCompleted) +
        sessionData.accuracy * sessionData.wordsCompleted) /
      tierData.wordsAttempted;
   ...
   */
  async updateBossBattleData(userId: string, sessionData: BossBattleSessionData): Promise<void> {
    const profile = await this.loadProfile(userId);
    if (!profile) {
  ...
  async updateAIPerformanceData(
    ...
    typeData.successRate =
      (typeData.successRate * (typeData.count - 1) + (interventionData.successful ? 1 : 0)) /
      typeData.count;

    // Update accuracy comparisons
    const accuracyData = interventionData.wasAISession
      ? aiData.accuracyImprovements.withAI
      : aiData.accuracyImprovements.baseline;
    accuracyData.sessions++;
    accuracyData.avgAccuracy =
      (accuracyData.avgAccuracy * (accuracyData.sessions - 1) + interventionData.afterAccuracy) /
      accuracyData.sessions;
    accuracyData.avgTime =
      (accuracyData.avgTime * (accuracyData.sessions - 1) + interventionData.afterTime) /
      accuracyData.sessions;

    // Calculate learning velocity (improvement per session)
    if (
      aiData.accuracyImprovements.withAI.sessions > 0 &&
      aiData.accuracyImprovements.baseline.sessions > 0
    ) {
      aiData.learningVelocity.withAI =
        aiData.accuracyImprovements.withAI.avgAccuracy /
        aiData.accuracyImprovements.withAI.sessions;
      aiData.learningVelocity.baseline =
        aiData.accuracyImprovements.baseline.avgAccuracy /
        aiData.accuracyImprovements.baseline.sessions;
    }
   ...
   */
  async updateQuickDashData(userId: string, sessionData: QuickDashSessionData): Promise<void> {
    const profile = await this.loadProfile(userId);
    if (!profile) {
    ...
    quickDashData.averageTimePerWord =
      (quickDashData.averageTimePerWord * (quickDashData.totalSessions - 1) +
        sessionData.timePerWord) /
      quickDashData.totalSessions;

    // Update time performance
    quickDashData.timePerformance.avgCompletionTime =
      (quickDashData.timePerformance.avgCompletionTime * (quickDashData.completedSessions - 1) +
        sessionData.totalTime) /
      quickDashData.completedSessions;

    if (
      sessionData.completed &&
      (quickDashData.timePerformance.fastestCompletion === 0 ||
        sessionData.totalTime < quickDashData.timePerformance.fastestCompletion)
    ) {
    ...
    if (sessionData.pressurePoints.length > 0) {
      const pressureAccuracy =
        sessionData.pressurePoints
      ...
      quickDashData.pressureResponse.accuracyUnderPressure =
        (quickDashData.pressureResponse.accuracyUnderPressure * (quickDashData.totalSessions - 1) +
          (pressureAccuracy || sessionData.accuracy)) /
        quickDashData.totalSessions;

      const lastMinutePoints = sessionData.pressurePoints.filter(p => p.timeRemaining < 60);
      if (lastMinutePoints.length > 0) {
        const lastMinutePerf =
          lastMinutePoints.reduce((sum, p) => sum + p.accuracy, 0) / lastMinutePoints.length;
        quickDashData.pressureResponse.lastMinutePerformance =
          (quickDashData.pressureResponse.lastMinutePerformance *
            (quickDashData.totalSessions - 1) +
            lastMinutePerf) /
          quickDashData.totalSessions;
      }
    }
   ...
   */
  async updateDeepDiveData(userId: string, sessionData: DeepDiveSessionData): Promise<void> {
    const profile = await this.loadProfile(userId);
    if (!profile) {
    ...
    deepDiveData.averageRetentionRate =
      (deepDiveData.averageRetentionRate * (deepDiveData.totalSessions - 1) +
        sessionData.retentionRate) /
      deepDiveData.totalSessions;

    deepDiveData.contextualLearningScore =
      (deepDiveData.contextualLearningScore * (deepDiveData.totalSessions - 1) +
        sessionData.contextualLearningScore) /
      deepDiveData.totalSessions;

    // Update repetition optimization
    if (sessionData.retentionRate > 0.8) {
      // Good retention indicates optimal repetition
      deepDiveData.repetitionOptimization.optimalRepetitionCount = Math.round(
        (deepDiveData.repetitionOptimization.optimalRepetitionCount + sessionData.repetitionCount) /
          2
      );
    }
  ...
  async updatePrecisionModeData(
    ...
    if (sessionData.failurePoint > 0) {
      precisionData.averageFailurePoint =
        (precisionData.averageFailurePoint *
          (precisionData.totalSessions - precisionData.perfectSessions - 1) +
          sessionData.failurePoint) /
        (precisionData.totalSessions - precisionData.perfectSessions);
    }

    // Update accuracy
    precisionData.zeroPressureAccuracy =
      (precisionData.zeroPressureAccuracy * (precisionData.totalSessions - 1) +
    ...
    if (sessionData.mistakeDetails) {
      ...
      if (precisionData.errorPatterns.mistakeProgression.length > 50) {
        precisionData.errorPatterns.mistakeProgression =
          precisionData.errorPatterns.mistakeProgression.slice(-50);
      }
    }
    ...
    if (sessionData.completed) {
      ...
      if (
        !precisionData.perfectSessionStrategies.effectiveQuizModes.includes(
          sessionData.quizModeUsed
        )
      ) {
      ...
      if (
        !precisionData.perfectSessionStrategies.cognitiveLoadManagement.includes(
          sessionData.cognitiveLoadStrategy
        )
      ) {
        precisionData.perfectSessionStrategies.cognitiveLoadManagement.push(
          sessionData.cognitiveLoadStrategy
        );
      }
    }
  ...
  async updateFillInTheBlankData(
    ...
    fillBlankData.contextualAccuracy =
      (fillBlankData.contextualAccuracy * (fillBlankData.totalSessions - 1) +
        sessionData.contextualAccuracy) /
      fillBlankData.totalSessions;

    fillBlankData.averageContextUtilization =
      (fillBlankData.averageContextUtilization * (fillBlankData.totalSessions - 1) +
        sessionData.contextUtilization) /
      fillBlankData.totalSessions;

    // Update context learning preferences
    if (sessionData.contextualAccuracy > 0.8) {
      // High accuracy indicates good complexity level
    ...
    fillBlankData.contextLearning.contextualClueEffectiveness =
      (fillBlankData.contextLearning.contextualClueEffectiveness *
        (fillBlankData.totalSessions - 1) +
        sessionData.contextualClueUsage) /
      fillBlankData.totalSessions;

    fillBlankData.contextLearning.grammarPatternRecognition =
      (fillBlankData.contextLearning.grammarPatternRecognition * (fillBlankData.totalSessions - 1) +
        sessionData.grammarRecognition) /
      fillBlankData.totalSessions;

    // Update comprehension insights
    fillBlankData.comprehensionInsights.semanticAccuracy =
      (fillBlankData.comprehensionInsights.semanticAccuracy * (fillBlankData.totalSessions - 1) +
        sessionData.semanticAccuracy) /
      fillBlankData.totalSessions;

    fillBlankData.comprehensionInsights.syntacticAccuracy =
      (fillBlankData.comprehensionInsights.syntacticAccuracy * (fillBlankData.totalSessions - 1) +
        sessionData.syntacticAccuracy) /
      fillBlankData.totalSessions;

    fillBlankData.comprehensionInsights.pragmaticAccuracy =
      (fillBlankData.comprehensionInsights.pragmaticAccuracy * (fillBlankData.totalSessions - 1) +
        sessionData.pragmaticAccuracy) /
      fillBlankData.totalSessions;
  ...
  async updateCrossModeAnalytics(userId: string): Promise<void> {
    ...
    crossMode.modePreferences = modes
      .map(mode => {
        const modeData = challenges[mode as keyof typeof challenges];
        if (
          typeof modeData === 'object' &&
          'totalSessions' in modeData &&
          'completedSessions' in modeData
        ) {
          const completionRate =
            modeData.totalSessions > 0 ? modeData.completedSessions / modeData.totalSessions : 0;
    ...
    const completionRates = modes
      .map(mode => {
        ...
        if (
          ...
          return modeData.totalSessions > 0
            ? modeData.completedSessions / modeData.totalSessions
            : 0;
        }
        return 0;
      })
    ...
    if (completionRates.length > 1) {
      ...
      const variance =
        completionRates.reduce((sum, rate) => sum + Math.pow(rate - avgCompletion, 2), 0) /
  ...
  async updateChallengeInsights(userId: string): Promise<void> {
    ...
    if (totalSessions > 5) {
      ...
      insights.optimalChallengeLevel = Math.min(
        100,
        Math.max(20, avgStreakPerformance * 30 + bossCompletionRate * 40 + aiSuccessRate * 30)
      );
    }
    ...
    if (recentCognitiveLoads.length > 0) {
      ...
      const avgCognitiveLoad =
        loadValues.reduce((sum: number, val: number) => sum + val, 0) / loadValues.length;
    ...
    if (streakData.cognitiveLoadPatterns.length > 0) {
      const highCognitiveLoadSessions = streakData.cognitiveLoadPatterns.filter(
        p => p.cognitiveLoad === 'high' || p.cognitiveLoad === 'overload'
      ).length;
  ...
  async getChallengeAnalytics(userId: string): Promise<{
    ...
    return {
      streakPerformance: {
        ...
        aiImprovementRate:
          challengeData.streakChallenges.aiEnhancedSessions > 0
            ? (challengeData.streakChallenges.aiEnhancedSessions /
                challengeData.streakChallenges.totalSessions) *
              100
            : 0,
      ...
      bossPerformance: {
        completionRate:
          challengeData.bossBattles.totalSessions > 0
            ? (challengeData.bossBattles.completedSessions /
                challengeData.bossBattles.totalSessions) *
              100
            : 0,
        averageWordsCompleted: challengeData.bossBattles.averageWordsCompleted,
        finalBossSuccessRate:
          challengeData.bossBattles.finalBossEncounters.attempts > 0
            ? (challengeData.bossBattles.finalBossEncounters.victories /
                challengeData.bossBattles.finalBossEncounters.attempts) *
              100
            : 0,
        phaseStrengths: Object.entries(challengeData.bossBattles.phasePerformance).sort(
          ([, a], [, b]) =>
            (b as { accuracy: number }).accuracy - (a as { accuracy: number }).accuracy
        ),
      },
      quickDashPerformance: {
        totalSessions: challengeData.quickDash.totalSessions,
        completionRate:
          challengeData.quickDash.totalSessions > 0
            ? (challengeData.quickDash.completedSessions / challengeData.quickDash.totalSessions) *
              100
            : 0,
        ...
        aiImprovementRate:
          challengeData.quickDash.aiEnhancedSessions > 0
            ? (challengeData.quickDash.aiEnhancedSessions / challengeData.quickDash.totalSessions) *
              100
            : 0,
      },
      deepDivePerformance: {
        ...
        completionRate:
          challengeData.deepDive.totalSessions > 0
            ? (challengeData.deepDive.completedSessions / challengeData.deepDive.totalSessions) *
              100
            : 0,
        ...
        aiImprovementRate:
          challengeData.deepDive.aiEnhancedSessions > 0
            ? (challengeData.deepDive.aiEnhancedSessions / challengeData.deepDive.totalSessions) *
              100
            : 0,
      },
      precisionModePerformance: {
        ...
        perfectSessionRate:
          challengeData.precisionMode.totalSessions > 0
            ? (challengeData.precisionMode.perfectSessions /
                challengeData.precisionMode.totalSessions) *
              100
            : 0,
        ...
        aiImprovementRate:
          challengeData.precisionMode.aiEnhancedSessions > 0
            ? (challengeData.precisionMode.aiEnhancedSessions /
                challengeData.precisionMode.totalSessions) *
              100
            : 0,
      },
      fillInTheBlankPerformance: {
        ...
        completionRate:
          challengeData.fillInTheBlank.totalSessions > 0
            ? (challengeData.fillInTheBlank.completedSessions /
                challengeData.fillInTheBlank.totalSessions) *
              100
            : 0,
        ...
        preferredComplexity:
          challengeData.fillInTheBlank.contextLearning.sentenceComplexityPreference,
        aiImprovementRate:
          challengeData.fillInTheBlank.aiEnhancedSessions > 0
            ? (challengeData.fillInTheBlank.aiEnhancedSessions /
                challengeData.fillInTheBlank.totalSessions) *
              100
            : 0,
      },
      ...
      aiEffectiveness: {
        interventionSuccessRate:
          challengeData.aiPerformance.totalInterventions > 0
            ? (challengeData.aiPerformance.successfulInterventions /
                challengeData.aiPerformance.totalInterventions) *
              100
            : 0,
        accuracyImprovement:
          challengeData.aiPerformance.accuracyImprovements.withAI.avgAccuracy -
          challengeData.aiPerformance.accuracyImprovements.baseline.avgAccuracy,
        learningVelocityGain:
          challengeData.aiPerformance.learningVelocity.withAI -
        ...
        mostEffectiveInterventions: Object.entries(challengeData.aiPerformance.interventionTypes)
          .sort(
            ([, a], [, b]) =>
              (b as { successRate: number }).successRate -
              (a as { successRate: number }).successRate
          )
          .slice(0, 3),
