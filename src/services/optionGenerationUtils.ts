 ...
 */
export function generateModuleScopedOptions(word: Word, languageCode: string, allWords: Word[]): string[] {
...
export function generateScrambledVersions(
  ...
  for (let i = 0; i < count + 2; i++) { // Generate extra to ensure we get enough unique ones
...
function scrambleRandom(chars: string[], swapCount: number, preserveStructure: boolean): string {
  ...
  for (let i = 0; i < swapCount; i++) {
    ...
    if (preserveStructure && Math.random() < 0.7) {
      ...
      pos1 = Math.max(1, Math.min(result.length - 2, middle - range + Math.floor(Math.random() * (range * 2))));
      pos2 = Math.max(1, Math.min(result.length - 2, middle - range + Math.floor(Math.random() * (range * 2))));
    }
