    ...
    <>
      ...
      <PerformancePanel isVisible={isVisible}>
        <MetricRow>
          ...
          <MetricValue warning={metrics.renderTime > 100}>{metrics.renderTime}ms</MetricValue>
        </MetricRow>

        {metrics.memoryUsage > 0 && (
          <MetricRow>
            <MetricLabel>Memory:</MetricLabel>
            <MetricValue warning={metrics.memoryUsage > 100}>{metrics.memoryUsage}MB</MetricValue>
          </MetricRow>
        )}
        ...
        <MetricRow>
          ...
          <NetworkStatus online={isOnline}>{isOnline ? 'Online' : 'Offline'}</NetworkStatus>
        </MetricRow>

        <MetricRow>
          <MetricLabel>FPS:</MetricLabel>
          <MetricValue>{Math.round(1000 / (metrics.renderTime || 16))}</MetricValue>
        </MetricRow>
      </PerformancePanel>
    </>
  );
