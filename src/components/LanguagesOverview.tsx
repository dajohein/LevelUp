...
import { FaGlobeAmericas, FaCog } from 'react-icons/fa';
...
const ProgressDot = styled.div<{ progress: number }>`
  ...
  border-radius: 50%;
  background: ${props => 
    props.progress > 50 ? '#4caf50' : 
    props.progress > 20 ? '#ff9800' : 
    props.progress > 0 ? '#2196f3' : 'rgba(255, 255, 255, 0.3)'
  };
...
export const LanguagesOverview: React.FC = () => {
  ...
  return (
    <Container>
      <ManagementButton onClick={() => navigate('/profile')}>
        <FaCog size={14} />
        Manage Data
      </ManagementButton>
      ...
      <Header>
        ...
        <Subtitle>
          Master new languages through interactive learning. Track your progress, earn achievements, and level up your skills across multiple languages!
        </Subtitle>
        ...
        {overallStats.totalXP > 0 && (
          <StatsOverview>
            ...
            <StatBadge>
              <span>🌍</span>
              {overallStats.languagesStarted} Language{overallStats.languagesStarted !== 1 ? 's' : ''} Started
            </StatBadge>
          </StatsOverview>
        )}
      </Header>
      ...
      <LanguageGrid>
        {languages.map(({ code, info }) => {
          ...
          const totalWords = actualModules.reduce((sum, module) => sum + (module.words?.length || 0), 0);
              ...
              <CardContent>
                {languageXP > 0 ? (
                  ...
                  <ProgressSection>
                    ...
                    <ModuleCount>
                      {info.modules?.length || 0} module{(info.modules?.length || 0) !== 1 ? 's' : ''} available
                    </ModuleCount>
                  </ProgressSection>
                )}
              </CardContent>
