...
const getLanguageSpecificInsights = (languageCode: string, momentum: LearningMomentum): string[] => {
  const baseInsights = {
    de: [
...
const generateAdvancedAIInsights = (userId: string, languageCode: string): AIAnalyticsData => {
  ...
  const insights: AIInsight[] = [
    {
      type: 'momentum',
      severity: momentum.direction === 'improving' ? 'success' : momentum.direction === 'declining' ? 'warning' : 'info',
      ...
      recommendations: [{
        ...
        message: momentum.direction === 'improving' 
        ...
        implementation: {
          immediate: momentum.direction === 'declining' 
            ? ['Take 5-minute mindfulness break', 'Review last 3 learned words', 'Adjust session complexity']
            : ['Continue current learning pace', 'Focus on new vocabulary', 'Maintain concentration'],
          shortTerm: momentum.direction === 'improving' 
            ? [`Increase ${userFactors.sessionPreference} session length by 10%`, 'Add advanced grammar patterns', 'Explore cultural context']
            : ['Simplify word complexity', 'Increase review frequency', 'Focus on consolidation']
        }
      }]
    },
    {
      type: 'cognitive',
      severity: cognitiveLoad.level === 'overloaded' ? 'critical' : cognitiveLoad.level === 'optimal' ? 'success' : 'info',
      ...
      recommendations: [{
        type: 'intervention',
        urgency: cognitiveLoad.level === 'overloaded' ? 'critical' : cognitiveLoad.score > 75 ? 'high' : 'medium',
        message: cognitiveLoad.level === 'optimal' 
        ...
        implementation: {
          immediate: cognitiveLoad.level === 'overloaded' 
            ? ['Pause current session', 'Deep breathing (30 seconds)', 'Hydrate', 'Review easier words']
          ...
          shortTerm: cognitiveLoad.level === 'high' 
            ? ['Reduce session duration by 20%', 'Increase break frequency', 'Focus on review vs new content']
            : ['Optimize learning intensity', 'Consider advanced topics', 'Explore complex grammar']
        }
      }]
    }
  ];

  const motivationLevel = Math.floor(userFactors.sustainabilityFactor * 100 + (Math.random() - 0.5) * 20);
  const churnRisk = Math.floor((1 - userFactors.sustainabilityFactor) * 40 + Math.random() * 15);

  const analytics: SessionAnalytics = {
    sessionsToday: Math.floor(1 + Math.random() * (userFactors.sessionPreference === 'intensive' ? 5 : 3)),
    avgSessionTime: Math.floor((userFactors.sessionPreference === 'intensive' ? 25 : 18) + Math.random() * 15),
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: '1.3rem', fontWeight: 'bold', marginBottom: '0.25rem', color: data.momentum.direction === 'improving' ? '#4CAF50' : data.momentum.direction === 'declining' ? '#FF9800' : '#2196F3' }}>
                {data.momentum.velocity}/hr
              </div>
              <div style={{ fontSize: '0.7rem', color: 'rgba(255, 255, 255, 0.7)', textTransform: 'uppercase', marginBottom: '0.25rem' }}>
                Learning Velocity
              </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '1.3rem', fontWeight: 'bold', marginBottom: '0.25rem', color: data.cognitiveLoad.level === 'optimal' ? '#4CAF50' : data.cognitiveLoad.level === 'overloaded' ? '#CF6679' : '#FF9800' }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '1.3rem', fontWeight: 'bold', color: data.motivationLevel > 80 ? '#4CAF50' : data.motivationLevel > 60 ? '#FF9800' : '#CF6679', marginBottom: '0.25rem' }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '1.3rem', fontWeight: 'bold', color: data.churnRisk < 20 ? '#4CAF50' : data.churnRisk < 40 ? '#FF9800' : '#CF6679', marginBottom: '0.25rem' }}>
                {data.churnRisk}%
              </div>
              <div style={{ fontSize: '0.7rem', color: 'rgba(255, 255, 255, 0.7)', textTransform: 'uppercase', marginBottom: '0.25rem' }}>
                Risk Level
              ...
              <div style={{ fontSize: '0.6rem', color: 'rgba(255, 255, 255, 0.5)' }}>
                {data.churnRisk < 20 ? 'low risk' : data.churnRisk < 40 ? 'moderate risk' : 'high risk'}
              </div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '1rem' }}>
            <h4 style={{ color: 'white', margin: '0 0 0.75rem 0', fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {data.insights.map((insight: AIInsight, index: number) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                  <div>
                    <h5 style={{ color: 'white', margin: '0 0 0.25rem 0', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {insight.title}
                      <span style={{ fontSize: '0.8rem' }}>{getUrgencyIcon(insight.recommendations[0]?.urgency || 'low')}</span>
                    </h5>
                    <p style={{ color: 'rgba(255, 255, 255, 0.8)', margin: 0, fontSize: '0.8rem', lineHeight: 1.4 }}>
                      {insight.description}
                    </p>
                  </div>
                  <div style={{
                  ...
                  <div style={{
                    ...
                    {insight.recommendations.map((rec, recIndex) => (
                      <div key={recIndex} style={{ marginBottom: '0.5rem' }}>
                        <div style={{ color: 'white', fontSize: '0.85rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>
                          💡 {rec.message}
                        </div>
                        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 0.5rem 0' }}>
                          {rec.reasoning.slice(0, 2).map((reason, reasonIndex) => (
                            <li key={reasonIndex} style={{
                              ...
                              <span style={{ position: 'absolute', left: 0, color: getSeverityColor(insight.severity) }}>→</span>
                              {reason}
                            </li>
                          ))}
                        </ul>
                        ...
                        {rec.implementation.immediate.length > 0 && (
                          <div style={{ marginTop: '0.5rem' }}>
                            <div style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '0.7rem', marginBottom: '0.25rem', textTransform: 'uppercase' }}>
                              Immediate Actions:
                            </div>
                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                              {rec.implementation.immediate.slice(0, 2).map((action, actionIndex) => (
                                <span key={actionIndex} style={{
                                  background: getSeverityColor(insight.severity),
                                  color: 'white',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#4a90e2', fontWeight: 'bold', fontSize: '1.1rem' }}>{data.analytics.sessionsToday}</div>
                <div style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Sessions Today</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#4a90e2', fontWeight: 'bold', fontSize: '1.1rem' }}>{data.analytics.avgSessionTime}m</div>
                <div style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Avg Session</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#4a90e2', fontWeight: 'bold', fontSize: '1.1rem' }}>{data.analytics.masteryScore}%</div>
          ...
          <div style={{ 
            ...
            User: {userId} | Language: {languageCode} | AI Confidence: {Math.floor(85 + Math.random() * 10)}%
          </div>
        </>
      )}
    </div>
