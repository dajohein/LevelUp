...
export const useAIEnhancedGame = (languageCode: string, moduleId?: string, enableAI = true) => {
  ...
  const loadNextWord = useCallback(async () => {
    ...
    try {
      ...
      if (enableAI && enhancedState.sessionEvents.length > 0) {
        try {
          const currentPerformance = simpleAILearning.analyzeSessionPerformance(
            enhancedState.sessionEvents
          );
          ...
          logger.debug(
            `🤖 AI selected quiz mode: ${finalQuizMode} (confidence: ${aiConfidence.toFixed(2)})`,
            {
   ...
   */
  const trackWordAttempt = useCallback(
        ...
        sessionProgress: prev.sessionProgress
          ? {
              ...
              correctAnswers: correct
                ? prev.sessionProgress.correctAnswers + 1
                : prev.sessionProgress.correctAnswers,
              currentIndex: prev.sessionProgress.currentIndex + 1,
              accuracy:
                (prev.sessionProgress.correctAnswers + (correct ? 1 : 0)) /
                (prev.sessionProgress.currentIndex + 1),
            }
          : null,
      }));

      logger.debug(
        `📊 Word attempt tracked: ${wordId} - ${correct ? 'correct' : 'incorrect'} (${responseTime}ms)`
      );
