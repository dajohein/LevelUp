...
_Status: Post Comprehensive Codebase Analysis_  
_Last Updated: December 30, 2025 - Comprehensive Code Quality Audit_
...
- **Console.log statements** - ✅ Not a priority (removed by build)
  - `vite-plugin-remove-console` already configured
  - Removes `console.log`, `console.debug`, `console.info` from production
  - Only `console.warn` and `console.error` remain in production
  - **Action**: Skip manual removal, focus on actual type safety
...
```bash
# Focus on actual blockers, NOT console.log
...
# Common issues:
# - Hooks called inside conditions
# - useState called after early returns
# - useEffect with missing dependencies

# Step 3: Fix empty interfaces
grep -r "interface.*{}" src/ --include="*.ts" --include="*.tsx"

# Step 4: Fix unescaped entities
grep -r "can't\|don't\|it's\|\"" src/components/ --include="*.tsx" | head -20

# Note: Console.log removal is automatic via vite-plugin-remove-console
# No need to manually remove - handled by build minification
```
...
removeConsole({
  includes: ['log', 'debug', 'info'], // ✅ Auto-removed from production
  // warn and error stay for production debugging
});

// So console.log statements in dev don't matter for:
// - Production bundle size
// - Performance
// - Debugging (they're stripped anyway)
```

**Target**: Remove all @ts-nocheck and fix underlying type safety by January 15, 2026
...
if (currentSession?.id === 'streak-challenge') {
  streakChallengeService
    .getNextStreakWord(currentStreak, wordProgress)
    .then(streakWord => {
      if (streakWord.word) {
        dispatch(
          setCurrentWord({
...
setTimeout(
  () => {
    quickDashService
      .getNextQuickDashWord(wordsCompleted, wordProgress, timeRemaining)
      .then(quickDashWord => {
        /* ... */
      });
  },
...
3. **🔧 Strategic @ts-nocheck Removal** (Day 1-3)
   - Focus on type safety fixes, NOT console.log
   - Console.log is handled by build minification (vite-plugin-remove-console)
   - Start with simplest file: one with fewest type issues
   - Follow: `remove @ts-nocheck → fix TypeScript errors → commit`
   - Pre-commit hooks will auto-format as you fix

   **Why not console.log?**
   - ✅ Already removed from production by `vite-plugin-remove-console`
   - ✅ Doesn't impact bundle size, performance, or production
   - ✅ Wasteful effort - build handles it automatically
   - ✅ Focus limited time on actual blockers (type safety, hooks)

4. **🎯 Understand Actual Blockers** (Critical)
   - React Hooks violations (conditional calls) - **FIXES STABILITY**
   - Type mismatches in interfaces - **FIXES RUNTIME ERRORS**
   - Missing type definitions - **FIXES TYPE SAFETY**
   - Not cosmetic things (console.log, formatting) - **BUILD HANDLES AUTOMATICALLY**
...
4. **Add observer cleanup**
   ...
   ```
5. **Day 4-5**: Async Performance Fixes
